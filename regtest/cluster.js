...
describe('aipgcoin Cluster', function() {
  ...
  before(function(done) {
    log.info('Starting 3 aipgd daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three aipgd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = aipgcore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.aipgd.once('tip', function(height) {
