...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for aipgd is necessary for this upgrade with the "reindex=1" aipg.conf option. \n' +
                 'There are changes necessary in both aipg.conf and aipgcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/underdarkskies/aipgcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed aipgcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './aipgcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in aipgcore-node services directory
    ...
    if (servicePackage.aipgcoreNode) {
      serviceModule = service.name + '/' + servicePackage.aipgcoreNode;
    }
 ...
 * @param {Node} node - The aipgcore Node instance
 */
