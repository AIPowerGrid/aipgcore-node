...
/**
 * Will remove a service from aipgcore-node.json
...
/**
 * Will remove the Node.js service and from the aipgcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The aipgcore-node configuration directory
...
function remove(options, done) {
  ...
  var aipgcoreConfigPath = path.resolve(configPath, 'aipgcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(aipgcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a aipgcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to aipgcore-node.json
        removeConfig(aipgcoreConfigPath, service, next);
      });
    }, done
  );
}
