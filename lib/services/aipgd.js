 ...
 */
function aipg(options) {
  if (!(this instanceof aipg)) {
    return new aipg(options);
  }
...
util.inherits(aipg, Service);

aipg.dependencies = [];

aipg.DEFAULT_MAX_TXIDS = 1000;
aipg.DEFAULT_MAX_HISTORY = 50;
aipg.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
aipg.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
aipg.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
aipg.DEFAULT_SPAWN_RESTART_TIME = 5000;
aipg.DEFAULT_SPAWN_STOP_TIME = 10000;
aipg.DEFAULT_TRY_ALL_INTERVAL = 1000;
aipg.DEFAULT_REINDEX_INTERVAL = 10000;
aipg.DEFAULT_START_RETRY_INTERVAL = 5000;
aipg.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
aipg.DEFAULT_TRANSACTION_CONCURRENCY = 5;
aipg.DEFAULT_CONFIG_SETTINGS = {
  assetindex: 1,
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
...
aipg.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || aipg.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || aipg.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || aipg.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || aipg.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || aipg.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || aipg.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || aipg.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || aipg.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || aipg.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || aipg.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

aipg.prototype._initCaches = function() {
...
aipg.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
aipg.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
aipg.prototype.getPublishEvents = function() {
  return [
    {
...
aipg.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
aipg.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
...
aipg.prototype.unsubscribeBalanceAll = function(emitter) {
...
aipg.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
...
aipg.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
 ...
 */
aipg.prototype.unsubscribeAddressAll = function(emitter) {
...
aipg.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = aipg.DEFAULT_CONFIG_SETTINGS;
...
aipg.prototype._parseaipgConf = function(configPath) {
  var options = {};
...
aipg.prototype._expandRelativeDatadir = function() {
...
aipg.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using aipg config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseaipgConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseaipgConf(networkConfigPath));
  }
...
aipg.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
...
aipg.prototype._resetCaches = function() {
...
aipg.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
aipg.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

aipg.prototype._initChain = function(callback) {
  var self = this;
...
aipg.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
...
aipg.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === aipgcore.Networks.testnet) {
...
aipg.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === aipgcore.Networks.testnet) {
...
aipg.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
...
aipg.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
aipg.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('aipg Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
aipg.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
aipg.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
aipg.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
aipg.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
aipg.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
...
aipg.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || aipg.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

aipg.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
aipg.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
aipg.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('aipg Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || aipg.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

aipg.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
aipg.prototype._stopSpawnedaipg = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned aipg process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean aipg process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
aipg.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedaipg(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting aipg process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('aipg process unexpectedly exited with code:', code);
        log.warn('Restarting aipg child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('aipg process restarted');
          });
...
aipg.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
aipg.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('aipg configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
aipg.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
aipg.prototype.syncPercentage = function(callback) {
  var self = this;
...
aipg.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
aipg.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
aipg.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
aipg.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
aipg.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
aipg.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
aipg.prototype._getConfirmationsDetail = function(transaction) {
...
aipg.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
aipg.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
aipg.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A aipg transaction id
 * @param {Function} callback
 */
aipg.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
aipg.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
aipg.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
aipg.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
aipg.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
aipg.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
aipg.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getRawBlock = function(blockArg, callback) {
...
aipg.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
aipg.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
aipg.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.getInfo = function(callback) {
  var self = this;
...
aipg.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipg.prototype.stop = function(callback) {
...
aipg.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
aipg.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
aipg.prototype.getMiningInfo = function(callback) {
    var self = this;
...
aipg.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
aipg.prototype.getPeerInfo = function(callback) {
    var self = this;
