...
/**
 * Provides a friendly event driven API to aipgd in Node.js. Manages starting and
 * stopping aipgd as a child process for application support, as well as connecting
 * to multiple aipgd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function aipgcoin(options) {
  if (!(this instanceof aipgcoin)) {
    return new aipgcoin(options);
  }
...
util.inherits(aipgcoin, Service);

aipgcoin.dependencies = [];

aipgcoin.DEFAULT_MAX_TXIDS = 1000;
aipgcoin.DEFAULT_MAX_HISTORY = 50;
aipgcoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
aipgcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
aipgcoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
aipgcoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
aipgcoin.DEFAULT_SPAWN_STOP_TIME = 10000;
aipgcoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
aipgcoin.DEFAULT_REINDEX_INTERVAL = 10000;
aipgcoin.DEFAULT_START_RETRY_INTERVAL = 5000;
aipgcoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
aipgcoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
aipgcoin.DEFAULT_CONFIG_SETTINGS = {
  assetindex: 1,
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
...
aipgcoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || aipgcoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || aipgcoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || aipgcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || aipgcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || aipgcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || aipgcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || aipgcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || aipgcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || aipgcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || aipgcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

aipgcoin.prototype._initCaches = function() {
...
aipgcoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
aipgcoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
aipgcoin.prototype.getPublishEvents = function() {
  return [
    {
...
aipgcoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'aipgd/' + name, 'total:', this.subscriptions[name].length);
};

aipgcoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'aipgd/' + name, 'total:', this.subscriptions[name].length);
};

aipgcoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (aipgcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'aipgd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

aipgcoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'aipgd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

aipgcoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
aipgcoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'aipgd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

aipgcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'aipgd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
aipgcoin.prototype.unsubscribeAddressAll = function(emitter) {
...
aipgcoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = aipgcoin.DEFAULT_CONFIG_SETTINGS;
...
aipgcoin.prototype._parseaipgcoinConf = function(configPath) {
  var options = {};
...
aipgcoin.prototype._expandRelativeDatadir = function() {
...
aipgcoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in aipgd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in aipgd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in aipgd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = (spawnOptions['configPath']) ? path.resolve(spawnOptions.configPath) : path.resolve(spawnOptions.datadir, './aipg.conf');

  log.info('Using aipgcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseaipgcoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseaipgcoinConf(networkConfigPath));
  }
...
aipgcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of aipgcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of aipgcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of aipgcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to aipgd from aipgcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from aipgd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from aipgd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in aipg.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that aipgd is undergoing a reindex. ' +
             ...
             'of aipgcore-node services will start.');
    node._reindex = true;
  }
};

aipgcoin.prototype._resetCaches = function() {
...
aipgcoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
aipgcoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

aipgcoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('aipgcoin Daemon Ready');
          callback();
        });
      });
...
aipgcoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === aipgcore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

aipgcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === aipgcore.Networks.testnet) {
    networkPath = 'testnet3/aipg.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/aipg.conf';
    }
  }
  return networkPath;
};

aipgcoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === aipgcore.Networks.testnet) {
...
aipgcoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('aipgd/hashblock', message.toString('hex'));
    }
  }

};

aipgcoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
aipgcoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('aipgcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
aipgcoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
aipgcoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
aipgcoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('aipgd/addressbalance', {
                    address: address,
                    txid: txid,
...
aipgcoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('aipgd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
aipgcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = aipgcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('aipgd/rawtransaction', message.toString('hex'));
    }
...
aipgcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || aipgcoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

aipgcoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
aipgcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
aipgcoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('aipgcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || aipgcoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

aipgcoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
aipgcoin.prototype._stopSpawnedaipgcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/aipgd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned aipgcoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean aipgcoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
aipgcoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedaipgcoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting aipgcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('aipgcoin process unexpectedly exited with code:', code);
        log.warn('Restarting aipgcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('aipgcoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn aipgd.'));
      }
...
aipgcoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to aipgd.'));
    }
 ...
 */
aipgcoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('aipgcoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
aipgcoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, aipgcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
aipgcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = aipgcore.Script.fromAddress(delta.address);
    return {
...
aipgcoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
aipgcoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
aipgcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in aipgcore is most recent to less recent
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
aipgcoin.prototype._getConfirmationsDetail = function(transaction) {
...
aipgcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
aipgcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
aipgcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A aipgcoin transaction id
 * @param {Function} callback
 */
aipgcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
aipgcoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof aipgcore.Address) {
...
aipgcoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
aipgcoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
aipgcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getRawBlock = function(blockArg, callback) {
...
aipgcoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a aipgcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
aipgcoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = aipgcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
aipgcoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
aipgcoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a aipgcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
aipgcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.getInfo = function(callback) {
  var self = this;
...
aipgcoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('aipgd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('aipgd process did not exit'));
      }
...
aipgcoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
aipgcoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
aipgcoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
aipgcoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
aipgcoin.prototype.getPeerInfo = function(callback) {
    var self = this;
